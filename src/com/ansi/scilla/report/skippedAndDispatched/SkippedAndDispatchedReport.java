package com.ansi.scilla.report.skippedAndDispatched;
...
public class SkippedAndDispatchedReport extends StandardReport implements ReportByDivMonthYear, ReportByDivision {

	private static final long serialVersionUID = 1L;
	public static final String FILENAME = "Skipped And Dispatched";

	private final String sql = "select concat(division_nbr,'-',division_code) as job_type\n"
			+ ", ticket.ticket_id\n"
			+ ", ticket.ticket_status\n"
			+ ", ticket.start_date\n"
			+ ", ticket.process_date\n"
			+ ", ''\n"
			+ ", isnull(ticket.process_notes,'')\n"
			+ ", ticket.act_price_per_cleaning\n"
			+ ", job.price_per_cleaning\n"
			+ ", ticket.invoice_date\n"
			+ ", isnull(fleetmatics_id,'') as FM_id\n"
			+ ", ticket_type\n"
			+ ", (select top 1 process_date  from ticket_history where ticket_history.ticket_id = ticket.ticket_id and ticket_status = 's'\n"
			+ "	order by history_date) as his_skip_date\n"
			+ ", invoice_style\n"
			+ ", ticket.job_id\n"
			+ "\n"
			+ "\n"
...
//	private List<RowData> data;
	 ...
	 * @throws Exception
	 */
	private SkippedAndDispatchedReport(Connection conn,  Integer divisionId, Calendar startDate) throws Exception {
		this();
...
//	    if ( this.data.size() == 0 ) {
	...
	public static XSSFWorkbook makeReport(Connection conn, Integer divisionId, Calendar startDate) throws Exception {
		SkippedAndDispatchedReport report = new SkippedAndDispatchedReport(conn, divisionId, startDate);
		return report.makeXLS();
	}
	
	public static SkippedAndDispatchedReport buildReport(Connection conn, Integer divisionId, Integer month, Integer year) throws Exception {
		GregorianCalendar startDate = new GregorianCalendar(year, month-1, 1);
		return new SkippedAndDispatchedReport(conn, divisionId, startDate);
	}
